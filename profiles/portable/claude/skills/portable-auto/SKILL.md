...
`/portable-auto` es automatico por definicion: acepta defaults seguros, no pide revisar asunciones y no pide confirmaciones normales antes de crear issue, crear PR o arrancar el loop. Solo frena ante errores duros, input realmente ambiguo o advertencias de tamano si no se paso `--continue-on-warning`.
...
```text
/portable-auto [--continue-on-warning] [--max N] <prompt|issue|url>
```
...
No agregar flags para aceptar defaults o confirmar acciones: ese comportamiento es implicito en `/portable-auto`.
...
```text
Working tree no esta limpio. /portable-auto crea branches y commitea; commitea o stashea los cambios pendientes antes de correr.
```
...
- Si `state == "CLOSED"`, abortar: `/portable-auto` no opera automaticamente sobre issues cerrados.
...
Redactar una spec usando la logica de `/portable-spec`, pero en modo auto:
...
El body del issue debe mantener la estructura de `/portable-spec`:
...
_Spec generada por `/portable-auto`._
```
...
Redactar un plan usando la logica de `/portable-plan`, pero en modo auto:
...
- Crear un unico archivo `.portable/plans/<SPEC_ISSUE>-<slug>.md`.
...
Branch: `portable-plan/<SPEC_ISSUE>`.
...
_Plan generado por `/portable-auto` a partir de #<SPEC_ISSUE>._
```
...
Ejecutar la logica de `/portable-code-loop` sobre `PLAN_PR` con `MAX_ITER`, pero en modo auto:
...
- Delegar exec al subagent `portable-code-executor`.
- Delegar validate al subagent `portable-code-validator`.
- Pasar el plan completo y feedback previo a los subagents.
- Aplicar labels `code:*` mutuamente exclusivos despues de cada fase.
- Postear cada validate report como comment con marker `<!-- portable-code-validate:feedback ... -->` via `--body-file`.
...
- Tratar `/portable-auto` como modo automatico por defecto.
