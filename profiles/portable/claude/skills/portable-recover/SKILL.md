Adopta issues y PRs preexistentes al workflow portable: detecta el tipo de entidad, diagnostica el estado actual (labels, artefactos, shape del body), genera los artefactos faltantes (`.portable/plans/<N>-<slug>.md`), commitea y pushea al branch del PR, aplica el label `entity:*` correspondiente, y sugiere el siguiente comando del workflow. `$ARGUMENTS` es el identificador del issue o PR (numero, `#N` o URL completa). Si viene vacio, se pide en el primer turno.
...
- Aplicar only labels `entity:*`; nunca tocar `code:*`.
- Output en espanol.
