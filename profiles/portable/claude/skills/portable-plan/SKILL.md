...
```
Working tree no esta limpio. /portable-plan crea una branch + PR; commiteá o stashé los cambios pendientes antes de correr.
```
...
Crear el directorio si no existe (`mkdir -p .portable/plans`) y escribir el archivo con `Write` tool (NUNCA via `echo` / heredoc — el contenido del issue puede tener caracteres que rompan).
...
```
Fallback: `PR_BODY_FILE="/tmp/cvm-portable-plan-pr-$(date +%s)-$$.md"`.
...
- Branch: `portable-plan/<issue-number>`. Path: `.portable/plans/<issue-number>-<slug>.md`. Body PR: `Closes #<N>`.
