...
| `/portable-auto` | (Claude Code only) Pipeline end-to-end automatico desde prompt o issue hasta PR validado: crea spec, crea plan y ejecuta code-loop sin confirmaciones normales. Soporta `--continue-on-warning` y `--max N` (default 5), y clasifica el fit final (`encajo limpio` / `encajo con friccion` / `encajo con riesgo residual` / `no encajo`). |
