...
| `/hs-auto` | (Claude Code only) Pipeline end-to-end autonomo desde prompt, issue o PR hasta PR validado. No depende de otros skills `/hs-*` ni de labels: redacta spec, plan y orquesta el loop exec/validate inline, delegando solo en los subagents `hs-code-executor` y `hs-code-validator`. En modo PR arranca siempre por validate y sintetiza `plan_text` in-memory si el branch no tiene `.harness/plans/*.md`. Aborta solo ante errores duros o prompts demasiado vagos para una spec minima. `--max N` (default 5). Clasifica el fit final (`encajo limpio` / `encajo con friccion` / `no encajo`). |
| `/detach` | (Claude Code only) Antesala para ejecutar una tarea en un subagent aislado. Recibe `$ARGUMENTS` como prompt en lenguaje natural, delega a `general-purpose`, y devuelve al main thread solo un bloque `## Result` de 3 lineas (status / summary / artifacts). Sirve para mantener limpio el contexto del thread principal cuando la tarea siguiente es ruidosa. |
| `/che-run` | Wrapper sobre `che run <slug> [prompt]` del CLI `che`. Hace preflight con `che doctor`, lista pipelines disponibles (`~/.che/pipelines/` + builtin `che-funnel`) si falta slug, corre el pipeline en background con `Monitor` para soportar runs largos, y reporta status leyendo el `manifest.yaml` del run mas reciente en `~/.che/runs/<slug>/`. Si el run fallo, vuelca las ultimas 30 lineas del stderr del primer step que fallo. Solo cubre `che run`; no toca `dash`, `upgrade` ni otros subcomandos. |
