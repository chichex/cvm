...
```
No hay un repo GitHub configurado en este directorio. /hs-plan necesita un repo para crear el PR final.
...
```
Working tree no esta limpio. /hs-plan crea una branch + PR; commiteá o stashé los cambios pendientes antes de correr.
```

### 3. Validar input (numero de issue)
- Si `$ARGUMENTS` esta vacio: pedir al usuario "Pasame el numero del issue de spec (ej: `42`)." y esperar respuesta. **No** continuar hasta tenerlo.
...
- Si `state == "CLOSED"`: avisar y preguntar `El issue #<N> esta cerrado. Querés generar el plan igual? (si/no)`. Si dice no, abortar.
- Si los labels **no** incluyen `entity:spec`: avisar `El issue #<N> no tiene label entity:spec. /hs-plan esta pensado para specs. Continuar igual? (si/no)`. Si dice no, abortar.
...
Para la barra de progreso, usar 10 segmentos: `▰` para completados (incluyendo el actual), `▱` para pendientes. Formula: `filled = round(k * 10 / M)`.
...
Crear el directorio si no existe (`mkdir -p .harness/plans`) y escribir el archivo con `Write` tool (NUNCA via `echo` / heredoc — el contenido del issue puede tener caracteres que rompan).
...
_Plan generado por `/hs-plan` a partir de #<N>._
```
...
### 3f. Crear PR
Generar body via `Write` a un tempfile (NUNCA interpolar en double-quoted commands):
```bash
PR_BODY_FILE="$(mktemp -t cvm-hs-plan-pr.XXXXXX).md"
```
Fallback: `PR_BODY_FILE="/tmp/cvm-hs-plan-pr-$(date +%s)-$$.md"`.
...
- Escribir el `.md` con `Write` tool (no heredoc en shell).
...
- Branch: `hs-plan/<issue-number>`. Path: `.harness/plans/<issue-number>-<slug>.md`. Body PR: `Closes #<N>`.
