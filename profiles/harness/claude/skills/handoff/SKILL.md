...
- Si `$ARGUMENTS` esta vacio, el foco se infiere de la conversacion en curso.
...
- Sin `$ARGUMENTS` o con hint tipo "seguir/continuar/retomar" → modo A.
- Con `$ARGUMENTS` que nombra una accion concreta + ref (path/PR/issue) → modo B.
- Ambiguo (no esta claro si quiere continuar todo o solo derivar la tarea) → preguntar con multiple choice (regla del profile: opciones numeradas + "otra"). Si el foco es vago de origen ("hace algo con el plan"), lanzar `/clarify` con `$ARGUMENTS` como input antes de seguir.
...
### 3. Detectar repo / branch / cwd (si aplica)
...
1. **Identificar referencias externas** de la conversacion (sin re-leer todo, solo lo que recuerdas del thread):
   - PRs abiertos / mencionados.
   - Issues abiertos / mencionados.
...
2. **Redactar el handoff con `Write`** al `$HANDOFF_FILE`:
   ...
   - Fecha: <TS UTC>
   - Origen: handoff desde sesion Claude Code
   ```
...
2. **Construir el prompt autocontenido** tailoreado al foco. Estructura sugerida (no rigida — adaptar al caso):
   ...
   ```text
   <objetivo concreto en imperativo, derivado de $ARGUMENTS>
...
- **Sugerir 1 a 3 skills** del profile activo que el proximo agente va a necesitar. Mirar `ls profiles/harness/claude/skills/` (o el profile que aplique) y elegir los relevantes a la tarea.
...
- Opcion B — nueva terminal: `claude "<prompt>"`.
```
