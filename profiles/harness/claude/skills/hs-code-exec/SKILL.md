Una sola pasada de implementacion sobre un PR con label `entity:plan`. Delega al subagent `hs-code-executor` (Sonnet) que escribe codigo segun el plan, hace chequeos minimos (build/typecheck + 1-3 unit tests acotados), commitea y pushea. NO valida — para eso esta `/hs-code-validate` o el loop completo `/hs-code-loop`. `$ARGUMENTS` es el numero de PR (puede venir vacio — en ese caso se pide).

Skill **exclusivo para Claude Code** (depende del subagent `hs-code-executor` en `~/.claude/agents/`). Wrapper thin sobre el agent — el orquestador solo hace pre-flight, delega, y reporta.
...
Si falla, abortar:
```
No hay un repo GitHub configurado en este directorio. /hs-code-exec necesita un PR de plan en GitHub.
```
...
```
Working tree no esta limpio. /hs-code-exec hace checkout de la branch del PR; commiteá o stashé los cambios pendientes antes de correr.
```
...
Voy a delegar a `hs-code-executor` (Sonnet) para una sola pasada de implementacion. Sin validacion al final — usa `/hs-code-validate <PR>` o `/hs-code-loop <PR>` para auditar.
...
- `description: "hs-code-exec single-shot"`
- `prompt`:
...
Para validar: /hs-code-validate <PR>
...
- Delegar a `hs-code-executor` (no a `general-purpose`).
- Pasar el plan completo en el prompt.
- Imprimir el `## Exec report` tal cual.

## MUST NOT DO

- No validar nada vos mismo (no correr tests/checks). Es responsabilidad de `/hs-code-validate`.
