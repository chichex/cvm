Una sola pasada de validacion sobre un PR con label `entity:plan`. Delega al subagent `hs-code-validator` (Opus) que espera `gh pr checks`, contrasta el diff vs cada paso/archivo/riesgo del plan, corre la suite completa local, y emite verdict PASS|FAIL con feedback accionable. NO ejecuta — para implementar usar `/hs-code-exec` o el loop completo `/hs-code-loop`. `$ARGUMENTS` es el numero de PR (puede venir vacio — en ese caso se pide).

Skill **exclusivo para Claude Code** (depende del subagent `hs-code-validator` en `~/.claude/agents/`). Wrapper thin sobre el agent. Sirve para auditar PRs propios o ajenos sin tocar codigo.
...
Si falla, abortar:
```
No hay un repo GitHub configurado en este directorio. /hs-code-validate necesita un PR de plan en GitHub.
```
...
```
Working tree no esta limpio. /hs-code-validate hace checkout de la branch del PR para correr tests; commiteá o stashé los cambios pendientes antes de correr.
```
...
```
Buscar un unico `.harness/plans/<N>-<slug>.md`. Si no aparece exactamente uno, abortar. Guardar como `PLAN_FILE`.
...
Voy a delegar a `hs-code-validator` (Opus) para una sola pasada de validacion: espera `gh pr checks`, corre suite completa local, contrasta diff vs plan. Sin ejecucion — usa `/hs-code-exec <PR>` o `/hs-code-loop <PR>` para implementar.
...
- `description: "hs-code-validate single-shot"`
- `prompt`:
...
```bash
COMMENT_FILE="$(mktemp -t cvm-hs-code-validate-feedback.XXXXXX).md"
```
Body del comment:
```markdown
<!-- hs-code-validate:feedback iter=single-shot verdict=<verdict> -->
## Validate feedback (single-shot) — <verdict>

<validate_report>

---
_Posted by `/hs-code-validate`._
```
Postear:
```bash
...
<si FAIL:>
Para resolver el feedback: /hs-code-exec <PR>
Para loop completo (exec + validate iterativo): /hs-code-loop <PR>
</si>
<si PASS:>
...
- Delegar a `hs-code-validator` (no a `general-purpose`).
- Pasar el plan completo en el prompt.
- Imprimir el `## Validate report` tal cual.

## MUST NOT DO

- No ejecutar nada vos mismo (no escribir codigo, no commitear). Es responsabilidad de `/hs-code-exec`.
- No hacer mas de una pasada — para iterar usar `/hs-code-loop`.
