...
```
Working tree no esta limpio. /hs-spec crea branch + PR en su fase de plan; commiteá o stashé los cambios pendientes antes de correr.
```
...
Aplicar el protocolo de `/clarify` por segunda vez, con un set distinto de restricciones (T1-T7). Esta pasada **no usa `$ARGUMENTS`**: el material es el body del issue de spec recien creado.
...
```
Tratar el `body` devuelto como `PROMPT` para `/clarify` (modo prompt-like). Saltar la deteccion de modo. NO re-leer `$ARGUMENTS`: el spec ya fue refinado y persistido en Fase A y es la fuente unica para esta pasada.
...
Crear el directorio si no existe (`mkdir -p .harness/plans`) y escribir el archivo con `Write` tool (NUNCA via `echo` / heredoc — el contenido puede tener caracteres que rompan).
...
```
Fallback: `PR_BODY_FILE="/tmp/cvm-hs-spec-pr-$(date +%s)-$$.md"`.
...
- En Fase B, listar **solo** asunciones tecnicas/de implementacion; alimentar el `/clarify` con el body del issue recien creado (no con `$ARGUMENTS` cruda).
- Ofrecer opt-out post-Fase-A con default `si`. Si el usuario corta, terminar limpio con solo el issue.
- Pasar el body del issue via `--body-file` y el body del PR via `--body-file` (Write tool genera los archivos).
...
- No re-leer `$ARGUMENTS` en Fase B — el spec creado es la fuente.
