...
- `pr_number` y `branch`
- `plan_text` - contenido completo del `.harness/plans/<N>-<slug>.md` (fuente de verdad)
...
1. **PR body + comments + reviews**:
   ...
   ```
   - `body`: leer entero.
   - `comments`: issue-level comments. Tomar los **ultimos 30**. Ignorar (al validar) los que tengan marker `<!-- hs-code-validate:feedback` - son tus propios reportes previos. Los demas son reviewers humanos o el executor; tomalos como criterios adicionales.
