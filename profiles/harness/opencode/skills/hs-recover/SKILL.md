---
name: hs-recover
description: Adoptar issues y PRs preexistentes al workflow harness aplicando labels entity:* y generando artefactos faltantes
---

Adopta issues y PRs preexistentes al workflow harness: detecta el tipo de entidad, diagnostica el estado actual (labels, artefactos, shape del body), genera los artefactos faltantes (`.harness/plans/<N>-<slug>.md`), commitea y pushea al branch del PR, aplica el label `entity:*` correspondiente, y sugiere el siguiente comando del workflow. Los argumentos del skill son el identificador del issue o PR (numero, `#N` o URL completa). Si vienen vacios, se pide en el primer turno.
