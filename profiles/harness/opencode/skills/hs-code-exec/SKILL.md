---
name: hs-code-exec
description: Una sola pasada de implementacion sobre un PR con label entity:plan usando el agent hs-code-executor
---

Una sola pasada de implementacion sobre un PR con label `entity:plan`. Delega al agent `hs-code-executor`, que escribe codigo segun el plan, hace chequeos minimos (build/typecheck + 1-3 unit tests acotados), commitea y pushea. NO valida; para eso esta `/hs-code-validate` o el loop completo `/hs-code-loop`. Los argumentos del skill son el numero de PR (pueden venir vacios; en ese caso se pide).
...
```text
Working tree no esta limpio. /hs-code-exec hace checkout de la branch del PR; commitea o stashea los cambios pendientes antes de correr.
```
...
Voy a delegar a `hs-code-executor` para una sola pasada de implementacion. Sin validacion al final; usa `/hs-code-validate <PR>` o `/hs-code-loop <PR>` para auditar.
