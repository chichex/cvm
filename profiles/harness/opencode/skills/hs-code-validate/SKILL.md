---
name: hs-code-validate
description: Una sola pasada de validacion sobre un PR con label entity:plan usando el agent hs-code-validator
---

Una sola pasada de validacion sobre un PR con label `entity:plan`. Delega al agent `hs-code-validator`, que espera `gh pr checks`, contrasta el diff vs cada paso/archivo/riesgo del plan, corre la suite completa local y emite verdict PASS|FAIL con feedback accionable. NO ejecuta; para implementar usar `/hs-code-exec` o el loop completo `/hs-code-loop`. Los argumentos del skill son el numero de PR (pueden venir vacios; en ese caso se pide).
...
```text
Working tree no esta limpio. /hs-code-validate hace checkout de la branch del PR para correr tests; commitea o stashea los cambios pendientes antes de correr.
```
...
Voy a delegar a `hs-code-validator` para una sola pasada de validacion: espera `gh pr checks`, corre suite completa local, contrasta diff vs plan. Sin ejecucion; usa `/hs-code-exec <PR>` o `/hs-code-loop <PR>` para implementar.
