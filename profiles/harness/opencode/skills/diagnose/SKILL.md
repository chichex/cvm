---
name: diagnose
description: Loop disciplinado para diagnosticar bugs duros y regresiones de performance; reproducir, minimizar, hipotetizar, instrumentar, fixear y dejar un regression test. Usar cuando hay un bug dificil de reproducir o sin causa clara.
---
...
- Si los argumentos estan vacios, pedir al usuario que describa el bug (sintoma, condiciones, alcance).
...
Proponer al usuario UNA opcion concreta y construirla. Iterar hasta que:

- [ ] Reproduce el sintoma reportado, no uno parecido.
- [ ] Corre en menos de 30s (idealmente menos de 5s).
- [ ] Es deterministico (corre 5 veces, falla las 5 o pasa las 5).

Si el bug es no-deterministico, el goal NO es repro 100% sino subir la tasa de repro a 50% o mas (loops, stress, sleeps inyectados, paralelizar el trigger).
...
Listar 2 a 4 hipotesis sobre la causa raiz, en orden de probabilidad. Para cada una, definir un check que la confirme o refute usando el loop.
...
Aplicar el fix minimo que apaga el loop (rojo a verde).
...
- [ ] Falla con el codigo viejo (verificar con `git stash` mas correr mas `git stash pop`).
