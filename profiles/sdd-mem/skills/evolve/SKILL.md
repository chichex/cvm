...
Delegar la creacion al skill `/skill-create` pasandole como argumento el nombre y la descripcion del skill propuesto. NO reimplementar la logica de creacion — `/skill-create` tiene las reglas de estructura, estilo, y persistencia.

Registrar la deteccion en KB:
```bash
cvm kb put "evolve-[nombre]" --body "Patron detectado: [descripcion]. Skill /[nombre] propuesto y creado via /skill-create." --tag "evolve,skill" [--local]
```
