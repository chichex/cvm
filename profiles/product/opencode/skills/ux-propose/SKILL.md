---
name: ux-propose
description: Genera 3-4 propuestas de pantalla en HTML+Tailwind desde un prompt; varia un eje, respeta WCAG 2.2 AA y guarda en .ux/proposals/<slug>/.
---

Generar **3-4 propuestas de pantalla en HTML+Tailwind** a partir de los argumentos del skill, para que el usuario elija y/o combine. Cada propuesta varia en un eje declarado (no random), abre en browser, y respeta WCAG 2.2 AA built-in. Los argumentos son el prompt de la pantalla y pueden venir vacios.
...
**Contenido real, no lorem ipsum**: usar contenido plausible al dominio del prompt.

**No JavaScript salvo lo minimo**: estas son propuestas para alinear visualmente. Tooltips, dropdowns y modales pueden mostrarse en estado abierto via `data-state="open"` para que se vean sin clicks.
...
Preguntar: `Voy a generar <N> variantes en .ux/proposals/<slug>/. Confirmás? (si/no, default: si)`. Si acepta, si la carpeta `.ux/proposals/<slug>/` no existe, crearla con `mkdir -p .ux/proposals/<slug>/` antes de escribir. Luego crear cada `variant-<N>-<nombre>.html` y el `README.md` indice con el tool de edicion seguro disponible.

## Fase 5 — Reportar

Reportar skill, cantidad de variantes generadas, directorio, eje de variacion y design system usado.
...
- No generar variantes que se diferencian solo cosmeticamente.
- No usar JavaScript salvo absolutamente minimo.
- No usar lorem ipsum.
