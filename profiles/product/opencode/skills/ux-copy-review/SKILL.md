---
name: ux-copy-review
description: Revisa microcopy, labels, errores, empty states y tono en HTML, imagen o paste; propone reescrituras y guarda en .ux/copy/<slug>.md.
---

Review de **microcopy y claridad linguistica**: button labels, mensajes de error, empty states, helper text, consistencia de tono. Audita HTML, imagen, o paste directo. Devuelve hallazgos + reescrituras sugeridas. Los argumentos del skill son el target y pueden venir vacios.
...
Preguntar: `Confirmás que guardo el output en .ux/copy/<slug>.md? (si/no, default: si)`. Si acepta, si la carpeta `.ux/copy/` no existe, crearla con `mkdir -p .ux/copy/` antes de escribir. Luego crear el archivo con el tool de edicion seguro disponible.

## Fase 6 — Reportar

Reportar skill, tipo de target, tono, idioma, strings auditados, total de hallazgos, strings con reescrituras, file resultante y si se guardo.
