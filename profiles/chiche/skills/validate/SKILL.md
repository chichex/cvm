...
**Si codex esta disponible — usar codex + Claude subagent:**

Subagent 1 — `Agent(subagent_type: "general-purpose", model: "opus")` con rol reviewer:
...
- **MUST DO**: Leer el codigo involucrado. Buscar commits recientes en el area. Verificar tests existentes. Terminar con `## Key Learnings:`.
...
**Si codex NO esta disponible — usar 2 subagents con hipotesis distintas:**

Subagent 1 (hipotesis: DATOS/ESTADO) — `Agent(subagent_type: "general-purpose", model: "opus")` con rol reviewer:
...
- **EXPECTED OUTCOME**: Root cause con archivo(s), linea(s), y explicacion de POR QUE ocurre.
- **MUST DO**: Seguir el flujo de datos desde el input. Verificar estado en cada paso. Buscar race conditions o mutaciones inesperadas. Terminar con `## Key Learnings:`.
- **MUST NOT DO**: Proponer fixes. Editar archivos. Asumir que la logica esta mal.
- **CONTEXT**: [problema, archivos conocidos]

Subagent 2 (hipotesis: LOGICA/CONTROL FLOW) — `Agent(subagent_type: "general-purpose", model: "opus")` con rol reviewer:
...
- **EXPECTED OUTCOME**: Root cause con archivo(s), linea(s), y explicacion de POR QUE ocurre.
- **MUST DO**: Trazar el control flow paso a paso. Verificar condiciones y branches. Buscar edge cases no cubiertos. Terminar con `## Key Learnings:`.
